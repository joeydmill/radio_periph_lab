...
void print_benchmark(volatile unsigned int *periph_base)
{
    ...
    unsigned int bytes_transferred = 4*2048; // 4 bytes each read * 2048 reads
    float time_spent = (stop_time-start_time) / 125e6; // counts / (125000000 counts/second)
    float throughput = (bytes_transferred / 1e6) / time_spent; // bytes_transferred / ((stop_time-start_time) / 125.0e6); // 4*2048 bytes / ((stop_counts - start_counts)/125000000 counts/sec)
...
// first, get a pointer to the peripheral base address using /dev/mem and the function mmap
    ...
    printf("\r\n\r\n\r\nLab 6 Joey Miller - Custom Peripheral Demonstration\n\r");
